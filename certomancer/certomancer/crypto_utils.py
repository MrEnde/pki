...
from asn1crypto.keys import PublicKeyInfo, PublicKeyAlgorithm
...
class GostcryptoBackend(CryptoBackend):
    ...
    def load_private_key(
        ...
        if "gost" not in private.algorithm:
            return self.backend.load_private_key(key_bytes, password)
        else:
            curve = private.curve["public_key_param_set"].native
            signer = gostsignature.new(
                mode=gostsignature.MODE_256 if "256" in curve else gostsignature.MODE_512,
                curve=gostsignature.CURVES_R_1323565_1_024_2019[curve]
            )
            key = bytes(signer.public_key_generate(private["private_key"].native))

            public = PublicKeyInfo({
                'public_key': key,
                'algorithm': {
                    'algorithm': private.algorithm,
                    'parameters': private["private_key_algorithm"]["parameters"]
                }
            })
    ...
    def load_public_key(self, key_bytes: bytes) -> keys.PublicKeyInfo:
        return self.backend.load_public_key(key_bytes)

    def generic_sign(self, private_key: keys.PrivateKeyInfo, tbs_bytes: bytes,
                     sd_algo: algos.SignedDigestAlgorithm) -> bytes:
        if "gost" not in sd_algo.signature_algo:
            return self.backend.generic_sign(private_key, tbs_bytes, sd_algo)

        parameters = private_key.curve

        md_algorithm = parameters["digest_param_set"].native
        curve = parameters["public_key_param_set"].native
        ...
        hasher = gosthash.new(
            name="streebog256" if "256" in md_algorithm else "streebog512",
            data=tbs_bytes
        )
        signature = signer.sign(
            private_key['private_key'].native,
            hasher.digest()[::-1]
        )

        return bytes(signature)

    def optimal_pss_params(self, key: PublicKeyInfo, digest_algo: str) -> algos.RSASSAPSSParams:
        return self.backend.optimal_pss_params(key, digest_algo)
...
CRYPTO_BACKEND: CryptoBackend = GostcryptoBackend(OscryptoBackend())
