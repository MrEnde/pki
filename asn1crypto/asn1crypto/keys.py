...
class NamedCurve(ObjectIdentifier):
    ...
    _key_sizes = {
        ...
        # Gost
        '1.2.643.7.1.2.1.1': 64,
        '1.2.643.7.1.2.1.1.1': 64,
        '1.2.643.7.1.2.1.1.2': 64,
        '1.2.643.7.1.2.1.1.3': 64,
        '1.2.643.7.1.2.1.1.4': 64,
        '1.2.643.7.1.2.1.2': 128,
        '1.2.643.7.1.2.1.2.0': 128,
        '1.2.643.7.1.2.1.2.1': 128,
        '1.2.643.7.1.2.1.2.2': 128,
        '1.2.643.7.1.2.1.2.3': 128,
        '1.2.643.2.2.35.1': 64
    }
...
class PrivateKeyInfo(Sequence):
    ...
    def _private_key_spec(self):
        ...
        return {
            ...
            'ed25519': OctetString,
            'ed448': OctetString,
            'id-tc26-gost3410-12-512': None,
            'id-tc26-gost3410-12-256': None
        }[algorithm]
    ...
    def curve(self):
        ...
        if self.algorithm != 'ec' and "gost" not in self.algorithm:
            raise ValueError(unwrap(
                '''
        ...
        if "gost" in self.algorithm:
            return self['private_key_algorithm']['parameters']
...
class PublicKeyInfo(Sequence):
    ...
    @property
    def gost256(self):
        if self._gost256 is not None:
            return self._gost256
        digest = gosthash.new("streebog256", data=byte_cls(self['public_key'])).digest()[:20]
        return byte_cls(digest)
